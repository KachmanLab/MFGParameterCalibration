...
def plot_results(results_dir, metadata):
    ...
    # 4. Error over time (Insights plot)
    for ds_name, ds_label in [("train", "Train"), ("test_id", "Test ID"), ("test_ood", "Test OOD")]:
        ...
        for name, data in models.items():
            ...
            # predictions is (days, N, d)
            preds = data[key]
            # compute mean squared error across all days at each time step
...
def main():
    ...
    for run_dir in glob.glob(os.path.join(RESULTS_DIR, "run_*")):
        if os.path.isdir(run_dir):
            print(f"Replotting for {run_dir}...")
            plot_results(run_dir, metadata)
